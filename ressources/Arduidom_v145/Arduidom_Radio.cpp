...
unsigned long RCSwitch::nReceivedAddr = NULL;
...
bool RCSwitch::receiveProtocol3(unsigned int changeCount) {
  ...
  for (int i = 2; i < 49 ; i = i + 2) {
    ...
    if  ( RCSwitch::timings[i + 1] > delay * 2 - delayTolerance
          && RCSwitch::timings[i + 1] < delay * 2 + delayTolerance) {
      codeaddr = codeaddr << 1;
    } else if (RCSwitch::timings[i + 1] > delay - delayTolerance
...
bool RCSwitch::receiveProtocol5(unsigned int changeCount) {
  ...
  unsigned long code = 0;
  unsigned long delay = RCSwitch::timings[0] / 14;
  unsigned long delayTolerance = delay * RCSwitch::nReceiveTolerance * 0.01;
